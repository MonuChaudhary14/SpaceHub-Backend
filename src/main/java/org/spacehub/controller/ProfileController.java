...
public class ProfileController {
  ...
  public ResponseEntity<?> deleteAccount(@RequestBody Map<String, String> payload, Authentication authentication) {
    try {
      ...
      return ResponseEntity.badRequest().body(Map.of("status", 400, "message", "Invalid input: " + e.getMessage()));
    }
    catch (SecurityException e) {
      return ResponseEntity.status(403).body(Map.of("status", 403, "message", "Incorrect password"));
    }
    catch (Exception e) {
      return ResponseEntity.internalServerError().body(Map.of("status", 500, "message", "An unexpected error occurred"));
    }
  }
