...
public class ChatRoomService {
  ...
  public ApiResponse<RoomResponseDTO> createRoom(CreateRoomRequest requestDTO) {
    ...
    RoomResponseDTO responseDTO = RoomResponseDTO.builder().roomCode(room.getRoomCode())
      .name(room.getName()).message("Room created successfully").build();
  ...
  public ApiResponse<ChatRoom> getRoomByCodeData(String roomCode) {
    ...
    return optionalRoom.map(chatRoom -> new ApiResponse<>(200,
        "Room fetched successfully", chatRoom))
  ...
  public ApiResponse<String> deleteRoomResponse(String roomCode, String userId) {
    ...
    if (userOpt.isEmpty()) return new ApiResponse<>(403, "You are not a member of this room",
      null);
    ...
    chatRoomUserService.getMembers(room).forEach(user ->
      chatRoomUserService.removeUserFromRoom(room, user.getUserId()));
    chatRoomRepository.delete(room);

    return new ApiResponse<>(200, "Room deleted successfully", "Room with code " + roomCode
      + " deleted.");
  }
  ...
  public ApiResponse<String> joinRoomResponse(String roomCode, String userId) {
    ...
    return new ApiResponse<>(200, "User added to room successfully, User " + userId + " added to room " +
      roomCode);
  }
  ...
  public ApiResponse<String> removeMember(RoomMemberAction requestDTO) {
    ...
    if (OptionalRequestor.isEmpty()) return new ApiResponse<>(403,
      "You are not a member of this room", null);
    ...
    if (OptionalTarget.isEmpty()) return new ApiResponse<>(404,
      "Target user not found in this room", null);
    ...
    if (requesterRole == Role.WORKSPACE_OWNER) {
      ...
      chatRoomUserService.removeUserFromRoom(room, requestDTO.getTargetUserId());
      return new ApiResponse<>(200, "Member removed successfully, User " + target.getUserId() +
        " removed from room " + room.getRoomCode());
    }
  ...
  public ApiResponse<String> changeRole(RoleChangeAction requestDTO) {
    ...
    if (requesterRole == Role.ADMIN) {
      ...
      return new ApiResponse<>(200, "Role updated successfully, User " + target.getUserId() + " is now " +
        target.getRole());
    }
